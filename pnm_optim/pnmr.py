...
class PNMR(Optimizer):
    r"""Implements Positive-Negative Momentum with restarts (PNMR).
    ...
    def __init__(self, params, lr=required, betas=(0.9, 1.), weight_decay=0, decoupled=True):
        ...
        super(PNMR, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(PNMR, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                step_scale = 2 ** int(math.log(state['step']) / math.log(2))
                beta2_hat = (beta2 + beta1 / (1. + beta1)) * math.cos( (state['step'] - step_scale)/step_scale * math.pi) - beta1 / (1. + beta1)
                    
                pos_momentum.mul_(beta1**2).add_(d_p, alpha=1-beta1**2)
                noise_norm = math.sqrt((1+beta2_hat) ** 2 + beta2_hat ** 2)
                delta_p = pos_momentum.mul(1+beta2_hat).add(neg_momentum, alpha=-beta2_hat).mul(1/noise_norm)
